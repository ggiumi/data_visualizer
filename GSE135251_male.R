...
if (file.exists("stored_objects_GSE135251_male.RData")) {
  load("stored_objects_GSE135251_male.RData")
} else {
  ...
  metaData_female <- subset(metaData_female, !is.na(nash_two) & nash_two != "control")
  metaData_male <- subset(metaData_male, !is.na(nash_two) & nash_two != "control")
  
  # Subset the countsData to include only the samples that are in the filtered metaData
  countsData_male <- countsData[, rownames(metaData_male)]
  countsData_female <- countsData[, rownames(metaData_female)]
  ...
  selected_colData <- metaData_male[c("disease", "fibrosis_stage", "group_in_paper", "nas_score", "Stage", "Sample_description", "sex")]
  
  save(dds, res, countsData_male, vsd_male, sampleDistMatrix, sampleDists, normalized_counts, colNames, cpm, counts_matrix, data, annotation, annotation_colors, colorbar, selected_colData, file = "stored_objects_GSE135251_male.RData")
}
...
createPCAPlot <- function(dds) {# Perform variance stabilizing transformation
  ...
  pcaData_male <- plotPCA(vsd_male, intgroup = "nash_two", returnData = TRUE)
  percentVar_male <- round(100 * attr(pcaData_male, "percentVar"))
  
  ggplot(pcaData_male, aes(PC1, PC2, color = nash_two)) +
    geom_point(size = 3) +
    xlab(paste0("PC1: ", percentVar_male[1], "% variance")) +
    ylab(paste0("PC2: ", percentVar_male[2], "% variance")) +
    coord_fixed() +
    theme_bw() +
    ggtitle("PCA Plot Male Samples")
}
